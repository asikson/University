...
def costFunction(mapping, linkMapping, physicalNetwork, virtualNetwork):
    cpuLoads = list(map(
        lambda m: round(virtualNetwork.getNode(m[0]).C / physicalNetwork.getNode(m[1]).C, 3,),
        mapping.items()
    ))

    memoryLoads = list(map(
        lambda m: round(virtualNetwork.getNode(m[0]).M / physicalNetwork.getNode(m[1]).M, 3),
        mapping.items()
    ))
